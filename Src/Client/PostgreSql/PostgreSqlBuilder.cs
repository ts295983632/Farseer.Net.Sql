...
namespace FS.Sql.Client.PostgreSql
{
    ...
    public class PostgreSqlBuilder : AbsSqlBuilder
    {
        ...
        public override ISqlParam ToList(int top = 0, bool isDistinct = false, bool isRand = false)
        {
            ...
            {
                Sql.Append($"SELECT {strTopSql}{strSelectSql}{randField} FROM {DbProvider.KeywordAegis(Name)} {strWhereSql} ORDER BY random()");
            }
            else
            {
                Sql.Append($"SELECT {strTopSql} *{randField} FROM (SELECT {strDistinctSql} {strSelectSql} FROM {DbProvider.KeywordAegis(Name)} {strWhereSql} {strOrderBySql}) s ORDER BY random()");
            }
            return this;
        }
